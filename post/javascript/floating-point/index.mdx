...
- 정확히 따지자면 자바스크립트에서 숫자 타입은 내부적으로 64비트 형식인 IEEE-754으로 표현되기 때문에 숫자를 저장하려면 64비트가 필요하다.
...
- <Link link="https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/Number/toFixed" target="_blank">toFixed()</Link> 메서드는 Number 객체를 주어진 매개변수 digits 만큼의 소수점 이하 자리수를 정확하게 갖는 문자열 표현으로 반환한다.
