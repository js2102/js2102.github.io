...
- Vue에서는 다음과 같은 경고를 띄운다.

  <Image src={vueResponsive2} />
...
- Vue.js는 개발자가 데이터 중심 방식으로 생각하고 DOM을 직접 건들지 않는 것을 권장한다.
- 하지만, DOM을 직접 건드려야 할 경우가 발생하는데 이 때는 Vue.nextTick()을 사용하면 된다.
...
- <Link link="https://v3.ko.vuejs.org/api/options-lifecycle-hooks.html" target="_blank">Vue 생명주기</Link> 메서드를 보면 created 메서드는 인스턴스가 생성된 후 동기적으로 호출된다. 즉, $el 속성을 사용할 수 없음을 의미한다.
- 따라서, 위 created 생명주기 메서드에서는 당연히 DOM 조작을 할 수 없게되고 위 콘솔은 undefined가 출력된다.
- 물론, $el 속성이 업데이트 되는 mounted 생명주기에서는 DOM을 조작할 수 있다.
- 하지만, 데이터 변경 후 DOM을 조작해야 한다면 이 또한 undefined가 출력된다. 이 때에는 Vue.nextTick()을 사용하면된다.
...
- 참고로 nextTick은 promise를 반환하므로 ES2017의 async/await 문법을 같이 사용할 수 있다.
